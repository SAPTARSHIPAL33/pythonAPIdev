...
def get_post(id:int,db:Session=Depends(get_db),user:int=Depends(oauth.get_the_user)): #, response: Response):   # id:int----I want the id to be integer
...
def delete(id:int,db:Session=Depends(get_db),user:int=Depends(oauth.get_the_user)):
